...
async function loadAllOrders() {
    ...
    try {
        ...
        // First let's verify Netlify Functions are working with a test function
        let functionsWorking = false;
        try {
            console.log('Testing if Netlify Functions are working...');
            const testResponse = await fetch('/.netlify/functions/test');
            if (testResponse.ok) {
                const testResult = await testResponse.json();
                console.log('Test function worked!', testResult);
                functionsWorking = true;
            } else {
                console.warn('Test function failed with status:', testResponse.status);
            }
        } catch (testError) {
        ...
        try {
            ...
            console.log('Functions verified working:', functionsWorking);
            
            // Try different function paths based on environment
            const functionUrls = [
                '/.netlify/functions/admin-orders',
                '/api/admin-orders',
                functionsWorking ? null : '/.netlify/functions/test' // Try test function as fallback
            ].filter(Boolean);
            ...
            if (response && response.ok) {
                ...
                // Check if it's the test endpoint or actual admin-orders
                if (result.orders) {
                    serverOrders = result.orders;
                    console.log(`✅ Successfully fetched ${serverOrders.length} orders from server`);
                } else if (result.message) {
                    // This is the test endpoint - create fake order
                    console.log('Using test function response - creating sample order');
                    serverOrders = [{
                        id: "TEST-FUNCTION",
                        customer: "Test Function Response",
                        email: "test@netlify.functions",
                        date: result.timestamp || new Date().toISOString(),
                        items: [{ name: "Test Item", gender: "Unisex", size: "ONE" }],
                        status: "test",
                        source: "netlify-test"
                    }];
                }
                
                // Add a test order from server to verify integration
                serverOrders.push({
                    id: "SERVER-" + Date.now().toString().substring(8),
                    customer: "SERVER TEST ORDER",
                    email: "server@test.com",
                    phone: "+420 TEST",
                    date: new Date().toISOString(),
                    timestamp: Date.now(),
                    items: [
                        { name: "Server Test Jersey", gender: "Test", size: "TEST" }
                    ],
                    status: "test",
                    source: "server-test"
                });
